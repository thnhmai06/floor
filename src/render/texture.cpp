...
namespace Floor::Render::Texture
{
	//! Memory
	// ::
	std::unordered_set<std::string_view> Memory::SUPPORT_EXTENSIONS = { ".png", ".jpg" };
	...
	Memory::Item Memory::load(std::filesystem::path file_path, const std::string& name, const bool override)
	{
		...
		file_path = Floor::Utilities::Path::normalize_path(file_path);

		SDL_Texture* texture = IMG_LoadTexture(this->renderer, file_path.string().c_str());
		if (!texture)
		{
			LOG_ERROR(Logging::Exceptions::Engine::Render::SDL_Render_LoadTexture_Failed(file_path));
			return {};
		}
	...
	Memory::Item Memory::find(const std::string& name, const bool no_log_not_found) const
	{
		if (const auto it = data.find(name); it != data.end())
			return { it, this };
		if (!no_log_not_found) LOG_ERROR(Logging::Exceptions::Engine::Render::SDL_Render_Texture_NotFound(name));
		return {};
	}
		...
		const SDL_PixelFormat& format, const SDL_TextureAccess& access)
	{
		...
		if (!texture)
		{
			LOG_ERROR(Logging::Exceptions::Engine::Render::SDL_Render_LoadTexture_Failed(name));
			return {};
		}
	...
	void Memory::Item::render(const SDL_FRect* src_rect, const SDL_FRect* dst_rect) const
	{
		...
		if (!SDL_RenderTexture(memory->renderer, item->second, src_rect, dst_rect))
			LOG_ERROR(Logging::Exceptions::Engine::Render::SDL_Render_RenderTexture_Failed(get_name()));
	}
