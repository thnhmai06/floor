#include "floor/events/action/render.h" // Header
#include <utilities.h>
#include <utility>

namespace Floor::Events::Action::Render
{
	//! Fade
	// FadeAction
	void FadeAction::update(const double& value_percent, const double& from, const double& to)
	{
		target->alpha.percent = Floor::Utilities::Math::to_value(value_percent, from, to);
	}
	...
	{
		target->render_pos.x = Floor::Utilities::Math::to_value(value_percent, from.x, to.x);
		target->render_pos.y = Floor::Utilities::Math::to_value(value_percent, from.y, to.y);
	}
	...
	{
		target->render_pos.x = Floor::Utilities::Math::to_value(value_percent, from, to);
	}
	...
	{
		target->render_pos.y = Floor::Utilities::Math::to_value(value_percent, from, to);
	}
	...
	{
		if (const auto it = dynamic_cast<Floor::Render::Config::ObjectConfig*>(target))
		{
			it->scale = {
				.x = Floor::Utilities::Math::to_value(value_percent, from.x, to.x),
				.y = Floor::Utilities::Math::to_value(value_percent, from.y, to.y) };
		}
	}
	...
	void RotateAction::update(const double& value_percent, const double& from, const double& to)
	{
		if (const auto it = dynamic_cast<Floor::Render::Config::ObjectConfig*>(target))
		{
			it->angle = Floor::Utilities::Math::to_value(value_percent, from, to);
		}
	}
	...
	void ColorAction::update(const double& value_percent, const Color& from, const Color& to)
	{
		if (const auto it = dynamic_cast<Floor::Render::Config::ObjectConfig*>(target))
		{
			it->color.r = Floor::Utilities::Math::to_value(value_percent, from.r, to.r);
			it->color.g = Floor::Utilities::Math::to_value(value_percent, from.g, to.g);
			it->color.b = Floor::Utilities::Math::to_value(value_percent, from.b, to.b);
		}
	}
	...
	ParameterAction::ParameterAction(
		...
		Floor::Render::Config::ObjectConfig& target, const Parameter& value,
